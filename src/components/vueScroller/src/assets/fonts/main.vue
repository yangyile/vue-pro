  ...
  <div class="vue-pull-to-wrapper" :style="{ height: wrapperHeight, transform: `translate3d(0, ${diff}px, 0)` }">
    ...
    <div v-if="topLoadMethod" :style="{ height: `${topBlockHeight}px`, marginTop: `${-topBlockHeight}px` }" class="action-block">
      <div class="top-load-wrapper">
        <img v-if="_topConfig.imgLoading" :src="_topConfig.imgUrl" />
        <p v-else>
    ...
    <div class="scroll-container">
      ...
      <div v-if="bottomLoadMethod" :style="{ height: `${bottomBlockHeight}px` }" class="action-block">
        <div class="bottom-load-wrapper">
          <spinner :style="{fill: '#aaa', stroke: '#aaa'}" v-if="state == 'loading'"></spinner>
          <div v-else>
            <svg class="icon" :class="{
                    'icon-arrow': state === 'trigger',
                    'icon-loading': state === 'loading',
...
import { TOP_DEFAULT_CONFIG, BOTTOM_DEFAULT_CONFIG } from "./src/config/config";
...
export default {
  ...
  methods: {
    ...
    handleScroll(event) {
      ...
      if (this.bottomReached) {
        this.$emit("bottom-state-change", "trigger");
      }
    },
  ...
  mounted() {
    this.init();
    console.log(this._topConfig, this._bottomConfig);
  }
};
</script>
